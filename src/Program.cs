            ...
            if (regexJson.IsMatch(home)) {
                    ...
                    if (trList != null) {
                        ...
                        foreach (var tr in trList) {
                            ...
                            if (tdList != null && tdList.Count == 7) {
                                ...
                                int result;
                                int.TryParse(tdList[4].InnerText.Trim(), out result);

                                if (diji == null || !string.IsNullOrWhiteSpace(tr.GetAttributeValue("flag", string.Empty))) {
                                    diji = JsonConvert.DeserializeObject<DataItem>(JsonConvert.SerializeObject(shengji));

                                    if (result == shengji.QuHuaDaiMa) {
                                        diji.QuHuaDaiMa = result + 100;
                                    } else {
                                        diji.QuHuaDaiMa = result;
                                    }

                                    diji.ParentId = shengji.QuHuaDaiMa;

                                    current = diji;
                                    current.DiJi = tdList[0].InnerText.Trim('+', '☆', ' ', '\r', '\n');
                                } else {
                                    current = JsonConvert.DeserializeObject<DataItem>(JsonConvert.SerializeObject(diji));

                                    current.QuHuaDaiMa = result;
                                    current.ParentId = diji.QuHuaDaiMa;
                ...
                using (var fs = File.OpenWrite(Path.Combine(dateDir, "China.csv"))) {
                    ...
                    using (var sw = new StreamWriter(fs, Encoding.UTF8)) {
                        sw.WriteLine("行政区划代码,父级代码,名称,简称,省级,地级,县级,区号,邮编");
                        foreach (var item in list) {
                            sw.WriteLine(string.Join(",",
                                item.QuHuaDaiMa,
                                item.ParentId,
                                item.Name,
                                item.ShengJiSortName,
                                item.ShengJiName,
                                item.DiJi,
                                item.XianJi,
                                item.QuHao,
